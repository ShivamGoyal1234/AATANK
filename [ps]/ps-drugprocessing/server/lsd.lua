	...
	if Player.Functions.RemoveItem("lsa", 1) then
		if Player.Functions.RemoveItem("thionyl_chloride", 1) then
			if Player.Functions.AddItem("lsd", 1) then
				...
				TriggerClientEvent('okokNotify', src, Lang:t("success.lsd"), "success")
			else
			...
			Player.Functions.AddItem("lsa", 1)
			TriggerClientEvent('okokNotify', src, Lang:t("error.no_thionyl_chloride"), "error")
		end
	else
		TriggerClientEvent('okokNotify', src, Lang:t("error.no_lsa"), "error")
	end
end)
	...
	if Player.Functions.RemoveItem("lsa", 1) then
		if Player.Functions.RemoveItem("chemicals", 1) then
			if Player.Functions.AddItem("thionyl_chloride", 1) then
				...
				TriggerClientEvent('okokNotify', src, Lang:t("success.thionyl_chloride"), "success")
			else
				Player.Functions.AddItem("lsa", 1)
