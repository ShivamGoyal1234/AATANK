...
RegisterNetEvent('ps-adminmenu:server:WarnPlayer', function(data, selectedData)
    ...
    if target ~= nil then
        ...
        TriggerClientEvent('okokNotify', source, locale("not_online"), 'error')
    end
end)
...
RegisterNetEvent('ps-adminmenu:server:ToggleBlackout', function(data)
    ...
    if Blackout then
        TriggerClientEvent('okokNotify', src, locale("blackout", "enabled"), 'primary')
        while Blackout do
            Wait(0)
            exports["qb-weathersync"]:setBlackout(true)
        end
        exports["qb-weathersync"]:setBlackout(false)
        TriggerClientEvent('okokNotify', src, locale("blackout", "disabled"), 'primary')
    end
end)
