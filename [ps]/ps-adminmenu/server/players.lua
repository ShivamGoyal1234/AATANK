...
RegisterNetEvent('ps-adminmenu:server:SetJob', function(data, selectedData)
    ...
    if not jobInfo then
        TriggerClientEvent('okokNotify', source, "Not a valid job", 'error')
        return
    end

    if not grade then
        TriggerClientEvent('okokNotify', source, "Not a valid grade", 'error')
        return
    end
...
RegisterNetEvent('ps-adminmenu:server:SetGang', function(data, selectedData)
    ...
    if not GangInfo then
        TriggerClientEvent('okokNotify', source, "Not a valid Gang", 'error')
        return
    end
