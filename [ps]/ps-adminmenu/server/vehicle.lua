...
RegisterNetEvent('ps-adminmenu:server:SaveCar', function(mods, vehicle, _, plate)
    ...
    if result[1] == nil then
        ...
        TriggerClientEvent('okokNotify', src, locale("veh_owner"), 'success', 5000)
    else
        TriggerClientEvent('okokNotify', src, locale("u_veh_owner"), 'error', 3000)
    end
end)
...
RegisterNetEvent('ps-adminmenu:server:FixVehFor', function(data, selectedData)
    ...
    if Player then
        ...
        TriggerClientEvent('okokNotify', src, locale("not_online"), "error")
    end
end)
