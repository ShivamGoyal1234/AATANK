...
GPT.Editable.GetFullWorkerInfo = function(identifier)
    ...
    if workerInfo then
        local xWorker = exports['qbx_core']:GetPlayerByCitizenId(identifier)
        if xWorker then
...
GPT.Editable.GetCaseData = function(id)
    ...
    for k, v in pairs(officers) do
        local xOfficer = exports['qbx_core']:GetPlayerByCitizenId(v.citizenid)
        if xOfficer then
...
GPT.Editable.AddCitizenLicense = function(license, name, player)
    ...
    if id then
        local player = exports['qbx_core']:GetPlayerByCitizenId(player)
...
GPT.Editable.FetchEmployees = function(jobName)
    ...
    for i = 1, #employees, 1 do
        ...
        local xEmployee = exports['qbx_core']:GetPlayerByCitizenId(employee.citizenid)
        local playerRadio = '-'
        if xEmployee then
...
GPT.Editable.Pay = function(data)
    local xPlayer = exports['qbx_core']:GetPlayer(data.player)
