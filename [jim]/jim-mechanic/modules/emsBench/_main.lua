    ...
    if DoesEntityExist(vehicle) then
        ...
        if Config.Emergency.PreformaceTable["Engine"] then
            ...
            Menu[#Menu+1] = {
                ...
                onSelect = function()
                    EmergencyBench.ChooseMod({ perform=true, id=id, header=locale("checkDetails", "engineLabel") })
                end,
            }
        end
        if Config.Emergency.PreformaceTable["Brakes"] then
            ...
            Menu[#Menu+1] = {
                ...
                txt   = "("..(possMods[id]+1)..") "..locale("common", "menuInstalledText")..": "..installed,
                onSelect = function()
                    EmergencyBench.ChooseMod({ perform=true, id=id, header=label })
                end,
            }
        end
