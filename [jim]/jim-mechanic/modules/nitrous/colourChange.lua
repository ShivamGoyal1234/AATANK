...
if Config.NOS.enable then
    ...
    onPlayerLoaded(function()
        NitrousColour.GetNosColourSync()
    end, true)

    -- Load stored nosColour on login
    NitrousColour.GetNosColourSync = function()
        nosColour = triggerCallback(getScript()..":GetNosColour")
        for plate in pairs(nosColour) do
            debugPrint("^5Debug^7: ^3nosColour^7[^6"..tostring(plate).."^7] = { ^2RBG^7: ^4" ..
            nosColour[plate][1].."^7, ^4"..nosColour[plate][2].."^7, ^4" ..
            nosColour[plate][3].." ^7}")
        end
    end
    ...
    --==========================================================
    AddStateBagChangeHandler("vehicleNitrousColour", nil, function(bagName, key, status)
        debugPrint("^5Debug^7: ^2Statebag change recieved^7: vehicleNitrousColour", json.encode(status))
        local entity = GetEntityFromStateBagName(bagName)
        if entity == 0 then return end
        if status ~= nil then
            nosColour[status.plate] = status.newColour
            debugPrint("^5Debug^7: ^2Receiving new ^3nosColour^7[^6"..
                        tostring(status.plate).."^7] = { ^2RBG: ^7= ^6"..
                        nosColour[status.plate][1].."^7, ^6"..nosColour[status.plate][2].."^7, ^6"..
                        nosColour[status.plate][3].." ^7}")
        end
    end)
