...
if Config.NOS.enable then
    ...
    -- Load stored VehicleNitrous on login
    Nitrous.GetNosSync = function()
        VehicleNitrous = triggerCallback(getScript()..":GetNosLoaded")
    ...
    --==========================================================
    AddStateBagChangeHandler("vehicleNitrous", nil, function(bagName, key, status)
        debugPrint("^5Debug^7: ^2Statebag change recieved^7: vehicleNitrous", json.encode(status))
        local entity = GetEntityFromStateBagName(bagName)
        if entity == 0 then return end
        if status ~= nil then
            debugPrint("^5Debug^7: ^2Updating ^4VehicleNitrous^7[^6"..status.plate.."^7]")
            VehicleNitrous[status.plate] = status.VehicleNitrous or nil

            local CurrentVehicle = GetVehiclePedIsIn(PlayerPedId())
            if DoesEntityExist(CurrentVehicle) then
                local CPlate = Helper.getTrimmedPlate(CurrentVehicle)
                if CPlate == status.plate then
                    ...
                    Helper.updateHudNitrous(
                        VehicleNitrous[status.plate] and VehicleNitrous[status.plate].level or 0,
                        VehicleNitrous[status.plate] and VehicleNitrous[status.plate].hasnitro or false,
                        false
                    )
                end
            end
    ...
    Nitrous.Keys.upLevel = function()
        local Plate = Helper.getTrimmedPlate(GetVehiclePedIsIn(PlayerPedId()))
        if VehicleNitrous[Plate]
           and VehicleNitrous[Plate].hasnitro
           and GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId()
        then
            -- Purge
