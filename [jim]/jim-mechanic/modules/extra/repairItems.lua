...
RegisterNetEvent(getScript()..":client:wheelRepair", function()
    ...
    if DoesEntityExist(vehicle) then
        ...
        if nearestWheel then
            ...
            local damaged = (IsVehicleTyreBurst(vehicle, nearestWheel.index, false) == 1) or (IsVehicleTyreBurst(vehicle, nearestWheel.index, true) == 1) -- or (GetVehicleWheelHealth(vehicle, nearestWheel.index) < 100.0)
            if not damaged then
                triggerNotify(nil, "wheel doesnt need fixing", "error")
                return
            end
        else
