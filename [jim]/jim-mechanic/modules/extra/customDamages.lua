onPlayerLoaded(function()
    local table = triggerCallback(getScript()..":callback:getStatusList")
    VehicleStatus = table
end, true)
...
--==========================================================
AddStateBagChangeHandler("vehicleStatus", nil, function(bagName, key, status)
    local entity = GetEntityFromStateBagName(bagName)
    debugPrint("^5Debug^7: ^2Recieving ^4VehicleStatus for entity: ^7'^6"..entity.."^7'")
    if entity == 0 or not status then return end
    if status ~= nil then
        debugPrint("^5Debug^7: ^2Updated ^4VehicleStatus^7[^6"..status.plate.."^7]")
        Helper.getDefalutHandling(entity, status.plate)
        VehicleStatus[status.plate] = status.VehicleStatus

        if Config.Stancer.enable then
            if VehicleStatus[status.plate].stancerEnabled == 1 then
                debugPrint("^5Debug^7: Adding veh ^3"..status.plate.." ^2to stance change^7")
                stanceToChange[entity] = status.plate
                Stancer.SetWheelStance(entity, status.plate)
            end
            if stanceToChange[entity] and VehicleStatus[status.plate].stancerEnabled == 0 then
